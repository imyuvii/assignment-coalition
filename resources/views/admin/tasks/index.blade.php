            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-Task">
                ...
                <tbody id="sortable">
                    @foreach($tasks as $key => $task)
                        <tr data-entry-id="{{ $task->id }}" class="sortable-item">
                            <td>
      ...
      $($.fn.dataTable.tables(true)).DataTable()
        ...
        $("#sortable").sortable({
            ...
            update: function(event, ui) {
                let sortedIDs = $(this).sortable("toArray", { attribute: 'data-entry-id' });
                $.ajax({
                    url: "{{ route('admin.tasks.updatePriority') }}", // Replace with your route
                    method: 'POST',
                    data: {
                        ids: sortedIDs,
                        _token: '{{ csrf_token() }}'
